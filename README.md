...
- **Card management**: editable icons (37 Lucide icons) and backgrounds (8 free photos); one card per app type (`comfyui` / `opencode` / `mdbook` / `vscode`), no paths involved (except the mdBook card, which points at a book project directory)
...
At startup the detected apps are printed, e.g. `detected apps: ComfyUI, opencode, MdBook, VS Code, VSCodium`; apps that are not installed never appear in the UI.
...
| POST | `/api/cards` | Create a card (`kind`: `comfyui`/`opencode`/`mdbook`/`vscode`/`vscodium`; 400 when the app is not installed, 409 on duplicate kind; `mdbook` also requires `dir`, a recognized project) |
...
internal/procman/  Backend abstraction, local instance lifecycle (ComfyUI server, opencode/mdbook/VS Code/VSCodium web), install & .app detection
