...
- **App detection**: the installed apps are detected at startup; **apps that are not installed are not shown** — no add entry, no create/start
  ...
  - `mdbook` cards serve an **mdBook documentation project** (`mdbook serve --port <n> --hostname 0.0.0.0`); the kind appears only when the **`mdbook` CLI is installed and `mdbook.dirs` is configured**. Picking the kind lists the books found one level deep under each configured directory (a book is a directory containing `book.toml`); a new book can be created there too (`mdbook init`). The chosen project directory is stored on the card. The book is **proxied same-origin under `/mdbook/`** on the dashboard listener (gated by your narthex session), so it is reachable through the same host, port and reverse proxy/tunnel as the dashboard — no extra port to expose
...
9. **Reorder**: drag a card to a new position to rearrange the grid — works with both mouse and touch (on touch, press and hold briefly until the card lifts, then drag); the new order is saved immediately and restored on the next visit
...
- `opencode.gatewayPort`: the fixed local port the narthex opencode gateway binds (the card "Open" address), default 4399; when it is occupied a free port is used instead. Editable in the opencode card's edit modal (applies immediately)
...
- `mdbook.hostname`: the internal mdBook server listen address, `0.0.0.0` by default; the browser always reaches the book through the same-origin `/mdbook/` proxy on the dashboard, so this only affects direct access to the backing server
- `mdbook.portRange`: the range the internal mdBook server ports are picked from, default [4500, 4699]
...
- **mdBook card won't start?** The **mdbook CLI** must be installed (`which mdbook`, or set `NARTHEX_MDBOOK_BIN`) and at least one directory configured in `mdbook.dirs`. The card points at a book project (a directory containing `book.toml`) picked or created in the add-card flow. Narthex runs `mdbook serve --port <n> --hostname 0.0.0.0` in the project directory; the port is picked from `mdbook.portRange` (default 4500–4699). The book is **proxied same-origin under `/mdbook/`** on the dashboard (session-gated), so the "Open" button works through the same host/port/reverse proxy as the dashboard. If the book fails to build, the card stays "Starting…" — check `logs/<cardID>.log`
