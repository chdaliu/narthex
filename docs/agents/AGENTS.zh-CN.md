...
| 路径 | 职责 |
|---|---|
| `cmd/narthex/main.go` | 入口:serve / setup / passwd / reset-auth / autostart 子命令与 flag;启动时打印检测到的应用(`detectedApps`);首次 serve 生成 opencode 密码与 VS Code/VSCodium 连接令牌(WeTTY 无需生成密钥);设置 vscode/vscodium 服务端数据目录(`Config.Vscode/Vscodium.DataDir`,默认 `<配置目录>/*-data`,展开 `~/`)并解析 vscode/vscodium 的 Machine 设置文件路径;端口占用预检;注入 `svc.GatewayHandler` 并在启动时调用 `ReconcileGateway`(若上次 serve 残留的 opencode 进程仍在运行,则恢复其网关);`resolveUsername`/`rotateSessionSecret`/`cmdAutostart`;注入 `svc.Restart`(自启任务已装且已加载 → `autostart.Restart` 走 launchd kickstart,否则 `restart.Self` 原地重执行) |
| `internal/api/service.go` | `Service` 结构(State + 互斥锁)、登录(用户名+密码)/会话/meta/settings/account 处理器、CardView 状态装配(经 Backend);检测接缝(`ComfyUIAppDir`/`ComfyUIDirs`/`OpencodeBin`/`MdbookBin`/`MdbookProjects`/`MdbookCreate`/`VscodeBin`/`VscodiumBin`/`WettyBin`);`HandleMeta` 返回 `apps`(kind → installed/label/reason,`appInfoFor` + `appUnavailableReason` 给出不可添加原因码 `add.reason.*`);卡片「打开」URL 装配 `instanceURL`(优先 `Config.InternalAddress`,否则请求主机;opencode 的 URL 指向 `GatewayPort` 网关,VS Code 系经 `codeToken` 追加 `?tkn=` 令牌;`directURL`/`CardView.APIURL` 暴露 opencode 直连端点供远程 API 客户端使用);`OpencodeGatewayTarget`(网关上游)、`Lang`、`GatewayPort`/`GatewayHandler`、`reconcileGatewayLocked`/`ReconcileGateway`(仅在 opencode 卡片存活时启动网关,否则停止);`IsLoopback`(仅 serve 警告用);`Restart func()`/`RestartDelay`/`BootID` 字段,`HandleRestart`(先回 200 再延迟执行 `Restart`;未接线 501),`HandleSession`(返回 `bootId`、`pageBackground` 及解析后的 `pageBackgroundUrl`,使登录页与仪表盘共用同一背景) |
| `internal/api/handlers.go` | 卡片 CRUD、start/stop;`appInstalled` 校验;`kindLabel` 默认名;`defaultIcon` 按 kind(palette/terminal/book-open/code);`installDir` 按 kind 解析(mdBook 用卡片 `dir`);每次变更与 `/api/cards` 轮询都调用 `reconcileGatewayLocked`,使网关跟随 opencode 卡片(进程被外部杀死时由轮询清理) |
...
| `internal/server/server.go` | 路由(Go 1.22+ method patterns)、鉴权中间件、`go:embed` 静态文件 + 缓存头(上传目录长缓存/`immutable`,背景预设长缓存);`GatewayHandler` 构建带鉴权的 opencode 反向代理 |
...
4. `make smoke` 输出 `SMOKE OK`(local 全流程(含首次运行无配置自动 setup) → **passwd 会话轮转**(旧 Cookie 401、新密码登录)→ **账号修改**(改用户名、旧会话 401、新用户名登录、旧用户名 401)→ **comfyui 卡片**(假 `Comfy Desktop.app` + `NARTHEX_COMFY_APP_DIR`,建卡/启动/URL 8000/停止/删除,重复 kind 409)→ **opencode 卡片**(假 `opencode` CLI + PATH 注入,建卡/启动/健康/停止/删除、密码生成并持久化、「打开」URL 指向网关(未认证 302、认证后代理 200)、直连 `apiUrl` 可访问)→ **mdbook projects API + 卡片**(假 `mdbook` CLI + `mdbook.dirs`,列出/新建书籍,卡片启动/健康/停止/删除)→ **vscode 卡片**(假 `code` CLI,建卡/启动/健康/停止/删除、令牌生成并持久化、`--server-data-dir` 目录已创建)→ **vscodium 卡片**(假 `codium` CLI,建卡/启动/健康/停止/删除、独立令牌生成并持久化、`--server-data-dir` 目录已创建、Machine 设置补齐且保留已有键)→ **wetty 卡片**(假 `wetty` CLI,建卡/启动/健康/停止/删除、无凭据)→ **autostart CLI**(status / 缺少操作 / 未知操作 / 缺少配置 错误路径,不实际 launchctl load)→ **restart 鉴权**(未认证 POST `/api/restart` 401,不触发重启))
