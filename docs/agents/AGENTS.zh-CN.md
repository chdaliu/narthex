...
- **启动卡片**:`api.HandleStartCard` → `Backend.Start(kind, dir, id)`;comfyui 的代码目录由 `procman.DetectComfyUIInstalls` 在启动时解析,并经 `procman.comfyLaunchArgs` 透传桌面端的共享模型路径配置(`--extra-model-paths-config`)、共享/实例输入输出目录(`--input-directory`/`--output-directory`)与实例额外启动参数(如 `--enable-manager`);opencode/VS Code/VSCodium/WeTTY 的工作目录为 `os.UserHomeDir()`;mdBook 的工作目录为卡片上存储的项目目录 → spawn(cwd=对应目录,`Setpgid: true`,日志写 `<LogDir>/<id>.log`)→ 记录 `pid/port/startedAt` 到 state;opencode 进程以 `OPENCODE_SERVER_PASSWORD` 环境变量携带 basic-auth 密码,VS Code/VSCodium 进程经 `--connection-token` 携带连接令牌,WeTTY 进程经 `--ssh-host`/`--ssh-port`/`--ssh-user` 携带 SSH 目标(无 HTTP 层密钥)
...
- **会话失效/陈旧标签页**:API 响应带 `Cache-Control: no-store`;前端在 focus/visibility/bfcache 恢复时重新校验 `GET /api/session`,失效则回到登录页并显示 `login.sessionExpired` 提示;opencode 网关监听在整个 serve 生命周期内绑定,因此陈旧的 opencode 标签页会得到登录重定向(会话失效)或本地化的「opencode 未运行」页(上游消失),而不是连接被拒的白页
- **mdBook 同源代理**:书籍经面板监听上的 `/mdbook/` 反向代理(见 `internal/gateway/mdbook.go`),卡片「打开」URL 为相对路径 `/mdbook/`;`MdbookGatewayTarget` 按请求解析运行中的 mdBook 卡片回环地址,未运行时返回本地化「mdBook 未运行」页;`/__livereload` 一并代理以实现实时刷新。因此无需额外暴露端口,单一反向代理/隧道即可访问书籍
...
| 路径 | 职责 |
|---|---|
| `cmd/narthex/main.go` | 入口:serve / setup / passwd / reset-auth / autostart 子命令与 flag;启动时打印检测到的应用(`detectedApps`);首次 serve 生成 opencode 密码与 VS Code/VSCodium 连接令牌(WeTTY 无需生成密钥);设置 vscode/vscodium 服务端数据目录(`Config.Vscode/Vscodium.DataDir`,默认 `<配置目录>/*-data`,展开 `~/`)并解析 vscode/vscodium 的 Machine 设置文件路径;端口占用预检;注入 `svc.GatewayHandler` 并在启动时调用 `ReconcileGateway`,为整个 serve 生命周期绑定 opencode 网关(上次 serve 残留的 opencode 进程仍在运行时也一并恢复);`resolveUsername`/`rotateSessionSecret`/`cmdAutostart`;注入 `svc.Restart`(自启任务已装且已加载 → `autostart.Restart` 走 launchd kickstart,否则 `restart.Self` 原地重执行) |
| `internal/api/service.go` | `Service` 结构(State + 互斥锁)、登录(用户名+密码)/会话/meta/settings/account 处理器、CardView 状态装配(经 Backend);检测接缝(`ComfyUIAppDir`/`ComfyUIDirs`/`OpencodeBin`/`MdbookBin`/`MdbookProjects`/`MdbookCreate`/`VscodeBin`/`VscodiumBin`/`WettyBin`);`HandleMeta` 返回 `apps`(kind → installed/label/reason,`appInfoFor` + `appUnavailableReason` 给出不可添加原因码 `add.reason.*`);卡片「打开」URL 装配 `instanceURL`(mdBook 返回同源相对路径 `/mdbook/`;其余优先 `Config.InternalAddress`,否则请求主机;opencode 的 URL 指向 `GatewayPort` 网关,VS Code 系经 `codeToken` 追加 `?tkn=` 令牌;`directURL`/`CardView.APIURL` 暴露 opencode 直连端点供远程 API 客户端使用);`OpencodeGatewayTarget`(opencode 网关上游)、`MdbookGatewayTarget`(运行中 mdBook 卡片的回环地址,供同源代理)、`Lang`、`GatewayPort`/`GatewayHandler`、`reconcileGatewayLocked`/`ReconcileGateway`(整个 serve 生命周期内绑定网关监听,不再随 opencode 卡片停止而关闭;`Shutdown` 释放监听);`IsLoopback`(仅 serve 警告用);`Restart func()`/`RestartDelay`/`BootID` 字段,`HandleRestart`(先回 200 再延迟执行 `Restart`;未接线 501),`HandleSession`(返回 `bootId`、`pageBackground` 及解析后的 `pageBackgroundUrl`,使登录页与仪表盘共用同一背景) |
| `internal/api/handlers.go` | 卡片 CRUD、start/stop、排序(`POST /api/cards/reorder` 按 `ids` 列表持久化卡片切片顺序;未知/重复 ID 忽略,未列出的卡片保持相对顺序并置于末尾);`appInstalled` 校验;`kindLabel` 默认名;`defaultIcon` 按 kind(palette/terminal/book-open/code);`installDir` 按 kind 解析(mdBook 用卡片 `dir`);每次变更与 `/api/cards` 轮询都调用 `reconcileGatewayLocked`,网关监听在首次调用时绑定并在整个 serve 生命周期内保持(`/api/cards` 轮询同时刷新卡片状态,进程被外部杀死会显示为已停止) |
...
| `internal/api/json.go` | JSON 读写辅助;所有 API 响应都带 `Cache-Control: no-store`,避免鉴权状态被缓存重放 |
...
| `internal/autostart/autostart.go` | macOS launchd 安装/卸载/查询/重启(用户级 serve);纯函数 `Label`/`PlistPath`/`Render` 加副作用 `Install`/`Uninstall`/`Status`/`Restart`(`launchctl kickstart -k`),通过包级函数变量接缝(`fnBootstrap`/`fnKickstart` 等)便于测试;非 darwin 返回 `ErrUnsupportedOS` |
| `internal/restart/restart.go` | `Self()`:解析 `os.Executable()`(`EvalSymlinks`+`Abs`)后以保留 argv/env 的 `syscall.Exec` 原地重执行(非 launchd 场景的回退);`execve` 为测试接缝 |
...
| `internal/procman/comfyui.go` | ComfyUI 服务器启动(`main.py --port --listen`,python 解析 `standalone-env`/`.venv`/PATH)、`comfyLaunchArgs`(透传桌面端共享模型配置 `--extra-model-paths-config`、共享/实例输入输出目录、`launchArgs`)、`DetectComfyUIInstalls`(从 `installations.json` 解析代码目录,`NARTHEX_COMFY_DESKTOP_DIR` 可覆盖)、`DetectComfyUIApp`(`Comfy Desktop.app`,`NARTHEX_COMFY_APP_DIR` 可覆盖) |
...
| `internal/server/server.go` | 路由(Go 1.22+ method patterns)、鉴权中间件、`go:embed` 静态文件 + 缓存头(上传目录长缓存/`immutable`,背景预设长缓存);`GatewayHandler` 构建带鉴权的 opencode 反向代理;`MdbookHandler` 构建同源 mdBook 代理并挂到 `/mdbook/` 与 `/__livereload`(单端口反向代理/隧道即可访问书籍) |
| `internal/gateway/mdbook.go` | 同源 mdBook 反向代理 `MdbookHandler`:`MdbookPrefix="/mdbook"` 挂载在面板监听上,要求 narthex 会话 Cookie(否则 302 回登录页),转发前剥离 `/mdbook` 前缀并透传 `/__livereload`,剥离客户端 Cookie;上游停止时返回本地化「mdBook 未运行」页;上游按请求实时解析 |
| `internal/gateway/gateway.go` | opencode 反向代理:`Handler` 要求 narthex 会话 Cookie(否则 302 回登录页),服务端注入 opencode 的 basic-auth `Authorization`,剥离客户端 Cookie,移除 `WWW-Authenticate`,透传 SSE(`FlushInterval=-1`)与 WebSocket;`Server` 管理监听生命周期——随 opencode 卡片启动、停止时关闭(`api.reconcileGatewayLocked`),不留任何 socket 或 goroutine;`Start` 优先绑定 `opencode.gatewayPort`(默认 4399),被占用时回退到内核分配的临时端口,使卡片「打开」URL 在重启后保持稳定;上游按请求实时解析,启动/停止即时生效 |
...
| `web/index.html`、`app.css`、`app.js` | 单页前端:登录/仪表盘/卡片/弹窗,玻璃拟态 + 响应式;添加弹窗无搜索步骤,`kindEntries()` 列出全部 6 类,不可添加的置灰并显示原因(未安装/ComfyUI 无实例/mdbook 无目录/已建卡片);选择 mdBook 类型后先进入「项目选择」步骤(在 `mdbook.dirs` 下选已有书籍或新建一本),再进入名称/图标/背景;共享的 `#bg` 元素在加载时由 `/api/session` 应用(登录页与仪表盘共用同一背景),写入 `localStorage`(`narthex.background`)实现首屏即时绘制并预加载,随 meta/设置刷新;opencode 卡片显示可复制的「API」行(`card.apiUrl`);卡片支持拖拽排序(`grid` 上的 pointerdown/pointermove/pointerup 状态机:鼠标/触控笔超过 6px 阈值即启动,触摸需长按 300ms;拖动时固定定位的 `.card-drag-ghost` 克隆跟随指针、真实卡片在 DOM 中移动,拖动开始后以非 passive 的 `touchmove` `preventDefault` 阻止触摸滚动,结束时把最终 DOM 顺序 POST 到 `/api/cards/reorder`;拖动期间暂停轮询,并吞掉尾随 click 以免打开卡片弹窗);设置弹窗含「重启 Narthex」按钮(`restartNarthex`:记录 `bootId` → POST `/api/restart` → 轮询 `/api/session` 直到 `bootId` 变化后 `location.reload()`) |
...
1. `gofmt -l .` 无输出
2. `go vet ./...` 无警告
3. `go test ./...` 全过:auth(哈希/会话/限速)、secretbox(加密往返/错误密钥/篡改/随机 nonce)、i18n(三语 key 完整性)、procman(桌面应用生命周期/进程扫描兜底/健康检查/内存/`.app` 检测/opencode 启停与报错/mdbook 检测·项目扫描·init/vscode·vscodium 检测与生命周期(含 `--server-data-dir` 与 Machine 设置补齐)/wetty 检测与生命周期)、store(默认值/规范化/原子写往返/按 kind 去重)、autostart(plist 渲染/路径解析/install/uninstall/status/restart 经 fakes,不真调 launchctl)、restart(`Self` 经 `execve` 接缝断言解析后的路径与 argv/env 透传)、api(fake Backend 全接口集成:鉴权/CRUD/生命周期/限速/meta(含 apps 与不可添加原因码)/语言切换/会话 TTL/账号修改/kind 校验与去重/应用未安装拒绝/内部组网地址 URL 替换/mdbook 卡片与 projects 端点/vscode·vscodium 令牌展示/wetty 卡片/会话 `bootId`/`POST /api/restart` 鉴权·501·调度调用)
4. `make smoke` 输出 `SMOKE OK`(local 全流程(含首次运行无配置自动 setup) → **passwd 会话轮转**(旧 Cookie 401、新密码登录)→ **账号修改**(改用户名、旧会话 401、新用户名登录、旧用户名 401)→ **comfyui 卡片**(假 `Comfy Desktop.app` + `NARTHEX_COMFY_APP_DIR`,建卡/启动/URL 8000/停止/删除,重复 kind 409)→ **opencode 卡片**(假 `opencode` CLI + PATH 注入,建卡/启动/健康/停止/删除、密码生成并持久化、「打开」URL 指向网关(未认证 302、认证后代理 200)、直连 `apiUrl` 可访问、停止后网关仍保持监听:未认证 302 / 认证后 503 本地化停用页并含面板链接)→ **mdbook projects API + 卡片**(假 `mdbook` CLI + `mdbook.dirs`,列出/新建书籍,卡片启动/健康/停止/删除,同源 `/mdbook/` 代理:未认证 302、认证后 200)→ **vscode 卡片**(假 `code` CLI,建卡/启动/健康/停止/删除、令牌生成并持久化、`--server-data-dir` 目录已创建)→ **vscodium 卡片**(假 `codium` CLI,建卡/启动/健康/停止/删除、独立令牌生成并持久化、`--server-data-dir` 目录已创建、Machine 设置补齐且保留已有键)→ **wetty 卡片**(假 `wetty` CLI,建卡/启动/健康/停止/删除、无凭据)→ **autostart CLI**(status / 缺少操作 / 未知操作 / 缺少配置 错误路径,不实际 launchctl load)→ **restart 鉴权**(未认证 POST `/api/restart` 401,不触发重启))
5. 前端改动需人工验证:登录页、卡片增删改、启动/停止、添加弹窗只列「已安装且未添加」的 kind、登录页与仪表盘共用同一背景(预设与上传图、已缓存)、opencode 网关免弹框打开、opencode 卡片「API」行、≤768px 移动端布局
...
- **settings 语义**:`POST /api/settings` 逐字段应用——`language`/`pageBackground` 传空串视为"未设置"(不修改);`slogan` 为 `*string`(`nil` 不修改,`""` 关闭,非空自定义 ≤80 字符);`internalAddress` 为 `*string`(`nil` 不修改,空串清除,自定义 ≤253 字符);`gatewayPort` 为 `*int`(`nil` 不修改,校验 1–65535 且 ≠ 面板端口;修改后经 `restartGatewayLocked` 立即重绑 opencode 网关);`nil` 且未设置时按当前语言解析 `i18n.T(lang, "slogan.default")`,自定义后不再随语言(见 `api.sloganValue`)
...
- **comfyui 服务器启动**:经 `procman.startComfyUI`(spawn `<python> main.py --port <n> --listen <host>`,cwd=安装代码目录,Setpgid),参数由 `procman.comfyLaunchArgs` 按 `installations.json` 中匹配的实例记录补齐——共享模型配置 `--extra-model-paths-config`(优先 `instance-model-paths/<id>.yaml`,回退 `shared_model_paths.yaml`)、`--input-directory`/`--output-directory`(读 `settings.json`,`useSharedInput/Output=false` 时用实例字段)、实例 `launchArgs`;代码目录无匹配记录时不加任何桌面参数;`appInstalled` 要求 `DetectComfyUIApp()` 与 `DetectComfyUIInstalls()` 均非空;监听地址与端口范围来自 `Config.ComfyUI`(`0.0.0.0` 默认,局域网可访问)
- **opencode web 启动**:经 `procman.startOpencode`(spawn `opencode web --port <n> --hostname <host>`,cwd=用户主目录,Setpgid);`appInstalled` 要求 `DetectOpencode()` 非空;**必须设置 `OPENCODE_SERVER_PASSWORD`**——服务绑定 `0.0.0.0`,无密码即局域网裸奔。密码首次 serve 时生成并持久化到 `Config.Opencode.Password`,经 `CardView.Password` 展示在卡片上。浏览器不直连 opencode:卡片「打开」URL 指向 narthex 网关(固定 `opencode.gatewayPort`,默认 4399;被占用时改用空闲端口,凭 narthex 会话 Cookie 鉴权),由服务端注入 basic-auth 凭据——既避免原生弹框,也绕开 opencode `crossorigin` 资源在 basic auth 下白屏的缺陷。网关监听在整个 serve 生命周期内绑定(`reconcileGateway`/`ReconcileGateway`,启动时及卡片变更时;不再随 opencode 停止而关闭,`api.Shutdown` 释放):opencode 未运行时返回本地化的「opencode 未运行」HTML 页并跳回面板,会话失效时重定向到面板登录页,因此陈旧的 opencode 标签页不会落到连接被拒的白页。`CardView.APIURL` 仍暴露直连 `http://<主机>:<端口>/` 端点(即卡片上的「API」行),使 `opencode attach`/SDK 等远程客户端可继续连接受密码保护的应用端口。监听地址与端口范围来自 `Config.Opencode`(`0.0.0.0`/`[4300, 4499]` 默认)
- **mdBook 服务器启动**:经 `procman.startMdbook`(spawn `mdbook serve --port <n> --hostname <host>`,cwd=书籍项目目录,Setpgid);`appInstalled` 要求 `DetectMdbook()` 非空**且 `Config.Mdbook.Dirs` 非空**——目录列表为空时 mdBook 类型整体隐藏;创建卡片要求 `dir` 属于 `DetectMdbookProjects(Config.Mdbook.Dirs)`;新建书籍仅允许在配置目录下经 `CreateMdbookProject`(`mdbook init --force --ignore none`,非交互)。监听地址与端口范围来自 `Config.Mdbook`(`0.0.0.0`/`[4500, 4699]` 默认)。浏览器经**同源代理**(`gateway.MdbookHandler`,挂载在面板监听的 `/mdbook/`)访问书籍:卡片「打开」URL 为相对路径 `/mdbook/`,`MdbookGatewayTarget` 按请求解析运行中卡片的回环地址(停止时返回本地化「mdBook 未运行」页);`/__livereload` 一并代理。无需额外暴露端口,单一反向代理/隧道即可访问书籍
